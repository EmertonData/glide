...
def _validate_y_proxy(y_proxy: NDArray, identifier: Optional[Hashable] = None) -> None:
    _validate_has_no_nan(y_proxy, "y_proxy")
    stratum_part = f" in stratum '{identifier}'" if identifier is not None else ""
    _validate_non_constant(y_proxy, f"'y_proxy' values are constant{stratum_part}.")


def _validate_y_true(y_true: NDArray, identifier: Optional[Hashable] = None) -> None:
...
def _validate_sample_sizes(
    ...
    if min(n_labeled, n_unlabeled) <= 1:
        stratum_part = f"stratum '{identifier}'" if identifier is not None else "dataset"
        raise ValueError(f"Too few labeled or unlabeled samples in {stratum_part}.")


def _validate_y_proxies(y_proxies: NDArray, identifier: Optional[Hashable] = None) -> None:
    if y_proxies.ndim != 2:
...
def _validate_min_samples(values: NDArray, name: str, identifier: Optional[Hashable] = None) -> None:
    if len(values) < 2:
        if identifier is not None:
            raise ValueError(
                f"'{name}' must have at least 2 non-NaN values per stratum; "
                f"got {len(values)} in stratum '{identifier}'."
            )
