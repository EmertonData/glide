...
class StratifiedPPIMeanEstimator:
    ...
    def estimate(
        ...
        for y_true_filled, y_proxy_labeled, y_proxy_unlabeled in strata:
            stratum_size = len(y_true_filled) + len(y_proxy_unlabeled)
            w_k = stratum_size / total_size

            lambda_k = _compute_tuning_parameter(y_true_filled, y_proxy_labeled, y_proxy_unlabeled, power_tuning)
            mean_k = _compute_mean_estimate(y_true_filled, y_proxy_labeled, y_proxy_unlabeled, lambda_k)
            std_k = _compute_std_estimate(y_true_filled, y_proxy_labeled, y_proxy_unlabeled, lambda_k)
