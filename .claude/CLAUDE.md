...
**`glide/mean_inference_results/`** — Result dataclasses returned by estimators (`MeanInferenceResult`, `ClassicalMeanInferenceResult`, `PredictionPoweredMeanInferenceResult`).
...
- `tests/unit/` mirrors the `glide/` folder structure exactly (e.g., `glide/mean_inference_results/foo.py` → `tests/unit/mean_inference_results/test_foo.py`)
