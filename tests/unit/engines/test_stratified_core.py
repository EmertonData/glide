...
import glide.engines.stratified_core as stratified_core_module
...
def test_preprocess_delegates(y_true, y_proxy, groups):
    with (
        ...
        np.testing.assert_array_equal(mock_validate_has_no_nan.call_args_list[1][0][0], y_proxy)
...
def test_preprocess_valid_output(y_true, y_proxy, groups):
    dataset = _preprocess(y_true, y_proxy, groups)

    assert set(dataset.keys()) == {"A", "B"}
    for stratum_id in ("A", "B"):
        y_true_filtered, y_proxy_labeled, y_proxy_unlabeled = dataset[stratum_id]
