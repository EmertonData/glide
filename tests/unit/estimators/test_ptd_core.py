...
def test_compute_ptd_bootstrap_labeled_means_returns_correct_shapes(rng):
    y_true = np.array([5.0, 6.0, 7.0])
    y_proxy_labeled = np.array([4.5, 5.5, 6.5])
    y_true_means, y_proxy_means = _compute_ptd_bootstrap_labeled_means(y_true, y_proxy_labeled, n_bootstrap=10, rng=rng)
    assert y_true_means.shape == (10,)
    assert y_proxy_means.shape == (10,)


def test_compute_ptd_bootstrap_labeled_means_with_pi(rng):
    y_true = np.array([5.0, 6.0, 7.0])
    y_proxy_labeled = np.array([4.5, 5.5, 6.5])
    pi = np.array([0.2, 0.5, 0.3])
    y_true_means, y_proxy_means = _compute_ptd_bootstrap_labeled_means(
...
def test_compute_ptd_bootstrap_mean_estimates_returns_correct_shape(rng):
    bootstrap_y_true_means = np.array([1.0, 2.0, 3.0])
    bootstrap_y_proxy_labeled_means = np.array([2.0, 4.0, 6.0])
...
def test_compute_ptd_bootstrap_mean_estimates_lambda_zero_returns_rectifier(rng):
