...
def y_arrays() -> Tuple[NDArray, NDArray, NDArray]:
    ...
    pi = np.array([0.4, 0.6, 0.5, np.nan, np.nan, np.nan])
...
def test_preprocess(estimator, y_arrays):
    ...
    y_true, y_proxy_labeled, y_proxy_unlabeled, pi_labeled = estimator._preprocess(y_true_all, y_proxy_all, pi_all)
    assert len(y_true) == 3
    assert len(y_proxy_labeled) == 3
    assert len(y_proxy_unlabeled) == 3
    assert len(pi_labeled) == 3
    assert not np.any(np.isnan(y_true))
    assert np.all(pi_labeled > 0)
...
def test_preprocess_raises_on_non_positive_pi(estimator):
    y_true = np.array([1.0, 2.0, np.nan, np.nan])
    y_proxy = np.array([0.9, 1.9, 0.8, 1.8])
    pi = np.array([0.5, 0.0, 0.5, 0.5])
    with pytest.raises(ValueError, match="Sampling probabilities must be > 0"):
        estimator._preprocess(y_true, y_proxy, pi)
...
def test_estimate_custom_confidence_level(estimator, y_arrays):
    ...
    expected_mean = 9.478
    expected_std = 0.576
    expected_lower = 8.737
    expected_upper = 9.991
...
def test_estimate_power_tuning_false(estimator, y_arrays):
    y_true, y_proxy, pi = y_arrays
    result = estimator.estimate(y_true, y_proxy, pi, n_bootstrap=5, power_tuning=False, random_seed=0)
    assert isinstance(result, PredictionPoweredMeanInferenceResult)
    assert np.isfinite(result.confidence_interval.mean)
...
def test_str_format(estimator, y_arrays):
    ...
    expected = (
        "Metric: performance\n"
        "Point Estimate: 9.478\n"
        "Confidence Interval (95%): [8.684, 10.003]\n"
