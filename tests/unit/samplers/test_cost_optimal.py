...
def test_sample_raises_if_fit_not_called(sampler, uncertainties):
    with pytest.raises(RuntimeError, match="Call fit\\(\\) before sample\\(\\)"):
        sampler.sample(uncertainties, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=5, random_seed=42)
...
def test_sample_delegates_to_validation(fitted_sampler, uncertainties):
    with (
        ...
        fitted_sampler.sample(uncertainties, y_true_cost=10.0, y_proxy_cost=0.0, max_cost=10, random_seed=42)
        ...
        mock_validate_bounds.assert_called_once_with(
            ...
            "max_cost",
            lower=10.0,
            error_message="'max_cost' should be at least 10.0; got 10.",
        )
...
def test_sample_negative_y_proxy_cost(fitted_sampler, uncertainties):
    with pytest.raises(ValueError, match="'y_proxy_cost' must be non-negative"):
        fitted_sampler.sample(uncertainties, y_true_cost=10.0, y_proxy_cost=-1, max_cost=5, random_seed=42)


def test_sample_known_output(fitted_sampler, uncertainties):
    pi, xi = fitted_sampler.sample(uncertainties, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=20, random_seed=42)
...
def test_sample_known_output_truncated_samples(fitted_sampler, uncertainties):
    pi, xi = fitted_sampler.sample(uncertainties, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=11, random_seed=2)
...
def test_sample_reproducibility(fitted_sampler, uncertainties):
    pi1, xi1 = fitted_sampler.sample(uncertainties, y_true_cost=1.0, y_proxy_cost=0.01, max_cost=1.5, random_seed=42)
    pi2, xi2 = fitted_sampler.sample(uncertainties, y_true_cost=1.0, y_proxy_cost=0.01, max_cost=1.5, random_seed=42)
...
def test_sample_different_seeds_results_differ(fitted_sampler, uncertainties):
    pi1, xi1 = fitted_sampler.sample(uncertainties, y_true_cost=1.0, y_proxy_cost=0.9, max_cost=20, random_seed=0)
    pi2, xi2 = fitted_sampler.sample(uncertainties, y_true_cost=1.0, y_proxy_cost=0.9, max_cost=20, random_seed=1)
