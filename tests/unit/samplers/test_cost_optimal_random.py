...
def test_sample_raises_if_fit_not_called(sampler):
    with pytest.raises(RuntimeError, match="Call fit\\(\\) before sample"):
        sampler.sample(n_samples=2, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=5, random_seed=42)
...
def test_sample_delegates_to_validation(fitted_sampler):
    with (
        ...
        fitted_sampler.sample(n_samples=2, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=10, random_seed=42)
        ...
        mock_validate_bounds.assert_called_once_with(
            ...
            "max_cost",
            lower=11.0,
            error_message="'max_cost' should be at least 11.0; got 10.",
        )


def test_sample_known_output(fitted_sampler):
    n_samples = 2
    pi, xi = fitted_sampler.sample(n_samples=n_samples, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=15, random_seed=42)
...
def test_sample_known_output_truncated_samples(fitted_sampler):
    n_samples = 5
    pi, xi = fitted_sampler.sample(n_samples=n_samples, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=11, random_seed=6)
...
def test_sample_reproducibility(fitted_sampler):
    pi1, xi1 = fitted_sampler.sample(n_samples=2, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=15, random_seed=42)
    pi2, xi2 = fitted_sampler.sample(n_samples=2, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=15, random_seed=42)
...
def test_sample_different_seeds_results_differ(fitted_sampler_high_MSE):
    pi1, xi1 = fitted_sampler_high_MSE.sample(
        n_samples=5, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=15, random_seed=0
    )
    pi2, xi2 = fitted_sampler_high_MSE.sample(
        n_samples=5, y_true_cost=10.0, y_proxy_cost=1.0, max_cost=15, random_seed=1
    )
