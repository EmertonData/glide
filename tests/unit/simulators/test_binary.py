...
def test_generate_binary_dataset_structure_and_counts():
    y_true, y_proxy = generate_binary_dataset(n_labeled=1, n_unlabeled=2, random_seed=0)
...
def test_generate_binary_dataset_invalid_true_mean_raises():
    with pytest.raises(ValueError, match=r"true_mean must be in \(0, 1\), got 1\.5"):
        generate_binary_dataset(n_labeled=1, n_unlabeled=1, true_mean=1.5)
...
def test_generate_binary_dataset_invalid_proxy_mean_raises():
    with pytest.raises(ValueError, match=r"proxy_mean must be in \(0, 1\), got 0"):
        generate_binary_dataset(n_labeled=1, n_unlabeled=1, proxy_mean=0.0)
...
def test_generate_binary_dataset_impossible_correlation_raises():
    with pytest.raises(
        ...
        generate_binary_dataset(n_labeled=1, n_unlabeled=9, true_mean=0.7, proxy_mean=0.6, correlation=0.95)


def test_generate_binary_dataset_reproducibility():
    y_true1, y_proxy1 = generate_binary_dataset(n_labeled=1, n_unlabeled=2, random_seed=7)
    y_true2, y_proxy2 = generate_binary_dataset(n_labeled=1, n_unlabeled=2, random_seed=7)
