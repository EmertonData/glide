...
def test_generate_binary_dataset_structure_and_counts():
    y_true, y_proxy = generate_binary_dataset(n_samples=3, random_seed=0)
...
def test_generate_binary_dataset_delegates_validation():
    with patch("glide.simulators.binary._validate_bounds") as mock_validate_bounds:
        generate_binary_dataset(n_samples=2, true_mean=0.7, proxy_mean=0.6, correlation=0.8, random_seed=0)
...
def test_generate_binary_dataset_impossible_correlation_raises():
    with pytest.raises(
        ...
        generate_binary_dataset(n_samples=10, true_mean=0.7, proxy_mean=0.6, correlation=0.95)


def test_generate_binary_dataset_reproducibility():
    y_true1, y_proxy1 = generate_binary_dataset(n_samples=3, random_seed=7)
    y_true2, y_proxy2 = generate_binary_dataset(n_samples=3, random_seed=7)
...
def test_generate_binary_dataset_different_seed_results_differ():
    y_true1, y_proxy1 = generate_binary_dataset(n_samples=10, random_seed=0)
    y_true2, y_proxy2 = generate_binary_dataset(n_samples=10, random_seed=1)
