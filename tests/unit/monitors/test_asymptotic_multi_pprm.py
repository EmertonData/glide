...
@pytest.fixture
def monitor():
    return AsymptoticMultiPPRM()


# --- detect ---


def test_detect_is_valid_monitoring_result(monitor, y_true, y_proxies, batches):
    ...
    assert result.monitor_name == "AsymptoticMultiPPRM"
    assert np.isfinite(result.running_means).all()
    assert (result.running_means >= result.confidence_bounds).all()


def test_detect_metadata(monitor, y_true, y_proxies, batches):
    result = monitor.detect(
        y_true, y_proxies, batches, higher_is_better=True, threshold=0.5, metric_name="accuracy", confidence_level=0.85
    )
