...
def test_detect_new_stratum_without_prior_history_uses_unpowered_estimate():
    """A stratum debuting alone in a batch, with no prior appearance in the stream, gets an unpowered estimate."""
    y_true_oracle, y_proxy, batches, groups = generate_batched_stratified_binary_dataset(
        n_samples=[[20], [20], [20]],
        true_mean=[[0.5], [0.5], [0.5]],
        proxy_mean=[[0.6], [0.6], [0.7]],
        correlation=[[0.6], [0.6], [0.6]],
        random_seed=0,
    )
    groups[batches == 2] = 1
    xi = np.tile(np.hstack([np.ones(8), np.zeros(12)]), 3)
    y_true = simulate_annotation(y_true_oracle, xi)

    monitor_result = AsymptoticStratifiedPPRM().detect(
        y_true, y_proxy, groups, batches, higher_is_better=False, threshold=0.5
    )
    last_batch_mask = batches == 2
    unpowered_estimate = StratifiedPPIMeanEstimator().estimate(
        y_true[last_batch_mask], y_proxy[last_batch_mask], groups[last_batch_mask], power_tuning=False
    )

    assert monitor_result.batch_mean_estimates[2] == pytest.approx(unpowered_estimate.mean)


def test_detect_prefix_consistency_with_stratum_gap():
    """A stratum absent from an intermediate batch and present again later stays prefix-consistent."""
    y_true_oracle, y_proxy, batches, groups = generate_batched_stratified_binary_dataset(
        n_samples=[[20, 20], [20], [20, 20], [20, 20]],
        true_mean=[[0.5, 0.5], [0.5], [0.5, 0.5], [0.5, 0.5]],
        proxy_mean=[[0.6, 0.65], [0.6], [0.6, 0.65], [0.6, 0.65]],
        correlation=[[0.6, 0.6], [0.6], [0.6, 0.6], [0.6, 0.6]],
        random_seed=0,
    )
    rng = np.random.default_rng(seed=1)
    xi = np.zeros(len(y_true_oracle))
    for batch_id in np.unique(batches):
        batch_indices = np.flatnonzero(batches == batch_id)
        labeled_indices = rng.choice(batch_indices, size=8, replace=False)
        xi[labeled_indices] = 1
    y_true = simulate_annotation(y_true_oracle, xi)
