...
**Note.** Each term in the formula above uses the **mean of cluster means** rather than a size-weighted mean, e.g. $\frac{1}{L^{\bullet}}\sum_l \bar{Y}^{(l)}$ rather than $\sum_l \frac{|C_l|}{n} \bar{Y}^{(l)}$. Both are unbiased for $\theta^*$, but their variances differ based on within-cluster dependence. Because no independence is assumed among observations within the same cluster, the conservative assumption treats them as perfectly correlated, which gives $\mathrm{Var}(\bar{Y}^{(l)}) = \sigma^2$ for every cluster, regardless of its size. Under this assumption, the variance of any estimator of the form $\sum_l \alpha_l \bar{Y}^{(l)}$ with $\sum_l \alpha_l = 1$ equals $\sigma^2 \sum_l \alpha_l^2$. By the Cauchy-Schwarz inequality, $\sum_l \alpha_l^2 \geq \frac{1}{L^{\bullet}}$ for all valid weight vectors, with equality when all weights are equal to $\frac{1}{L^{\bullet}}$. The mean of cluster means achieves this minimum, whereas the size-weighted mean has $\alpha_l = \frac{|C_l|}{n}$, yielding a strictly larger variance estimation whenever cluster sizes are unequal. The two estimated variances coincide only when all clusters have the same size.
