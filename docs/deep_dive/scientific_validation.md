...
Across Monte Carlo repetitions, we report the mean width and a percentile band to characterize both average efficiency and variability. The band spans from the $\lfloor \alpha / 2 \cdot 100 \rfloor$th to the $\lceil (1 - \alpha / 2) \cdot 100 \rceil$th percentile of the per-seed width distribution.
